...
import axios from "axios";function Signin() {
  const [loginForm, setloginForm] = useState({
    username: "",
    password: "",
  });
  const [Customer, setCustomer] = useState([]); // State to store user info  // const navigate = useNavigate(); // Hook for navigation
  ...
  const handleSubmit = async (e) => {
    ...
    const url = `http://localhost:5053/api/customer?username=${encodeURIComponent(
      loginForm.username
    )}&password=${encodeURIComponent(loginForm.password)}`;    try {
