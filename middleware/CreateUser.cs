    ...
    {  
        public Createuser(User user){
            using (SqlConnection conn  = new SqlConnection()){
                conn.ConnectionString = "Server=localhost;Port=1433;Database=master;Trusted_Connection=True;Data Source=.\\SQLEXPRESS";
                try{
                    conn.Open();
                }
                catch (Exception ex) {
            ...
            string sqlQuery = "INSERT INTO  Customer(firstName, lastName, username, password) values (@firstName, @lastName, @username, @password)"
